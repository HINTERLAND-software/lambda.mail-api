const fs = require('fs');
const {
  domains, mailgun, translations, defaultValidationFields
} = require('../config');
 ...
 * @returns {string}
 */
const processTemplate = (keys, type, locale = 'de', configDomain) => {
  ...
  const processed = template
    ...
    .replace('*|_DOMAIN_|*', configDomain ? `(${configDomain})` : '')
    .replace('*|_COPYRIGHT_|*', locales.copyright);
...
const callbackHandler = callback => (statusCode, message, input = '') => {
  ...
  if (process.env.NODE_ENV !== 'test') {
    console.log(JSON.stringify({ statusCode, message }, null, 2));
  }
...
const getSMTPconfig = (senderDomain) => {
  const {
    fallback: _, domain: configDomain, validationFields = defaultValidationFields, ...auth
  } =
