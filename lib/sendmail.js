...
const { templates, translations } = require('../config');
...
module.exports = async (smtpConfig, smtpTransport, keys, recipient) => {
  ...
  const { config, configDomain, validationFields } = smtpConfig;
  ...
  const mailOptions = {
    ...
    replyTo: keys.mail,
    text: processTemplate(keys, templates.txt, locale, configDomain), // plaintext body
    html: processTemplate(keys, templates.html, locale, configDomain) // html body
  };

  // send mail with defined transport object
  const result = `${Date()} | ${configDomain} | message "${validationFields.required
    .map(key => keys[key])
    .join('", "')}"`;
